...
#   4/8 ---------> 4
    ...
    def read(self):
        ...
        self.changeWorkingMode("ReceiveCON")
        return message
    # This function will receive payload from LoRa
    def receive(self, timeout):
        start_time = time.time()
        while True:
            # Check for incoming message
            irq_flags = self.readFromSPI(REG.REG_IRQ_FLAGS, 1)
            if irq_flags[0] & IRQMASKS.IRQ_RX_DONE_MASK:
                # Clear the IRQ flag for RX_DONE
                self.writeOnSPI(REG.REG_IRQ_FLAGS, IRQMASKS.IRQ_RX_DONE_MASK)

                # Read number of received bytes
                num_bytes = self.readFromSPI(REG.REG_RX_NB_BYTES, 1)[0]

                # Set FIFO pointer to the current RX address
                fifo_current_addr = self.readFromSPI(REG.REG_FIFO_RX_CURRENT_ADDR, 1)[0]
                self.writeOnSPI(REG.REG_FIFO_ADDR_PTR, fifo_current_addr)

                # Read the received message
                message = self.readFromSPI(REG.REG_FIFO, num_bytes)

                # Check CRC (if enabled)
                if self.CRC and not (irq_flags[0] & IRQMASKS.IRQ_PAYLOAD_CRC_ERROR_MASK):
                    return message
                elif not self.CRC:
                    return message
                else:
